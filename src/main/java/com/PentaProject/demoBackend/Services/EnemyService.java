...
public class EnemyService {
    ...
    public Enemy generateEnemy(String name, Integer health, Integer mana, Integer attack, Integer defense,
            Integer agility, Integer APs, Integer range, ClassType category) throws IllegalArgumentException {
    ...
    public List<Enemy> getEnemiesFromStage(int id) throws IOException {
        // de-serealizzazione del file json valuestage, contenente i parametri min-max
        // dei nemici
        StageSelector value = json.readValue(resourceFile.getInputStream(), StageSelector.class);
        Stage stage = value.getStages().stream().filter(x -> x.getId() == id).findFirst().orElseThrow();
        ...
        if (!boss) {
                    ...
                    .collect(Collectors.collectingAndThen(
                            ...
                            collected -> {
                                ...
                                for (int i = 0; i < clone; i++, j++, k++) {
                                    ...
                                    c.setDefense(
                                            random.nextInt(Integer.parseInt(defence[0]), Integer.parseInt(defence[1])));
                                    c.setAttack(
                                            random.nextInt(Integer.parseInt(attack[0]), Integer.parseInt(attack[1])));
                                    c.setAgility(
                                            random.nextInt(Integer.parseInt(agility[0]), Integer.parseInt(agility[1])));
            ...
            Enemy Benemy = template.stream()
                    .filter(x -> x.getName().startsWith("Admin") || x.getName().startsWith("Regitare")).findFirst()
                    .get();
